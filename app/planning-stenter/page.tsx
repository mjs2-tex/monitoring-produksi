...
import BaruEditModalSTENTER from "./modal/BaruEditModalSTENTER";
...
export default function PlanningPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch("/api/planning?type=STENTER");
  ...
  return (
    <div className="w-full h-[calc(100vh-70px)] flex flex-col p-4 gap-2 bg-slate-50 text-slate-800">
      {showModal && (
        <BaruEditModalSTENTER
          isOpen={showModal}
          onClose={() => {setShowModal(false); setMasterData([]);}}
          title="Tambah Data Planning STENTER"
          masterState={masterState}
          masterData={masterData}
          fetchData={fetchData}
        />
      )}
      <h2 className="text-2xl font-bold italic">| Planning STENTER</h2>
