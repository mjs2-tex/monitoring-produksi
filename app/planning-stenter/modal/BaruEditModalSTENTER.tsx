...
export default function BaruEditModalSTENTER({ isOpen, onClose, title, masterState, masterData = [], fetchData }: FullScreenModalProps) {
    if (!isOpen) return null;
    const router = useRouter();
    const [activeTab, setActiveTab] = useState("STENTER");
    ...
    const handleEdit = async () => {
        try {
            ...
            if (response.status === 201 || response.status === 200) {
                ...
                onClose();
                // // 6. Cleanup & Redirect
                // localStorage.removeItem("temp_master_STENTER");
                // localStorage.removeItem("temp_details_STENTER");
    ...
    const generateNomor = async (kategori: string) => {
        ...
        if (data.success) {
            ...
            // Contoh output: PLANNING-STENTER-0001
        }
    };
    useEffect(() => {
        loadData();
        if (masterState === "BARU") {
            ...
            const savedMaster = localStorage.getItem("temp_master_STENTER");
            const savedDetails = localStorage.getItem("temp_details_STENTER");
    ...
    useEffect(() => {
        if (masterState === "BARU") {
            const timeoutId = setTimeout(() => {
                localStorage.setItem("temp_master_STENTER", JSON.stringify(master));
                localStorage.setItem("temp_details_STENTER", JSON.stringify(details));
