...
export default function BaruEditModalINSPECT({ isOpen, onClose, title, masterState, masterData = [], fetchData }: FullScreenModalProps) {
    ...
    const handleEdit = async () => {
        try {
            ...
            if (response.status === 201 || response.status === 200) {
                ...
                onClose();
                // // 6. Cleanup & Redirect
                // localStorage.removeItem("temp_master_INSPECT");
                // localStorage.removeItem("temp_details_INSPECT");
    ...
    useEffect(() => {
        ...
        if (masterState === "BARU") {
            ...
            const savedMaster = localStorage.getItem("temp_master_INSPECT");
            const savedDetails = localStorage.getItem("temp_details_INSPECT");
    ...
    useEffect(() => {
        if (masterState === "BARU") {
            const timeoutId = setTimeout(() => {
                localStorage.setItem("temp_master_INSPECT", JSON.stringify(master));
                localStorage.setItem("temp_details_INSPECT", JSON.stringify(details));
